<?php
    ...
    function generatePointsWithinRadius($polygon,$radius = 50) { //Default radius is 50 meters
        //Finding number of free parking spaces at the specified simulation time
        $keyId = $polygon['id'];
        $keyDemand = $GLOBALS["demandData"][$keyId - 1]['demand']; //demandData array indexes are -1 the id of the polygon they contain (sql querry that calculates them is sorted by id)
        if($keyDemand >=1) {
    ...
    $validPolygons = getPolygonsWithinRadius($radius,$lat,$lng); //contains polygon_id and available parking spaces --also available global array $demandData
    
    foreach($validPolygons as $validPolygon) {
        $randomPoints = generatePointsWithinRadius($validPolygon);
        break;
        //dbscan()
    }
